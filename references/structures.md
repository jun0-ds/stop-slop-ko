...
AI가 생성한 텍스트를 그대로 복붙할 때 발생하는 구조적 문제.

| 패턴 | 문제 |
|------|------|
| `**텍스트**` 별표가 노출됨 | 마크다운 볼드가 렌더링 안 된 채로 남음 |
| `## 제목` 샵 기호 노출 | 마크다운 헤딩이 일반 텍스트에 그대로 |
| `- 항목` 하이픈 노출 | 불릿 포인트 문법이 그대로 보임 |
| `> 인용` 꺾쇠 노출 | 인용 문법이 렌더링 안 됨 |
| 번호 리스트 남용 | "1. 2. 3." 구조가 AI 티를 냄 |

**대신:** 일반 텍스트 환경(SNS, 메신저, 이메일)에서는 마크다운 문법 전부 제거. 강조가 필요하면 문장 구조로 강조할 것. 리스트 대신 문장으로 풀어 쓸 것.
