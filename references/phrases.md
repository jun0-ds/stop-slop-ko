...
AI가 쓴 글의 가장 눈에 띄는 물리적 증거. 복붙할 때 마크다운 문법이 그대로 남는다.

- `**볼드 처리**` 별표가 텍스트에 그대로 노출됨
- `## 헤딩` 샵 기호가 그대로 남음
- `- 불릿 포인트` 하이픈이 일반 텍스트에 노출됨
- `\`코드 블록\`` 백틱이 그대로 보임
- `[링크 텍스트](URL)` 마크다운 링크 문법이 노출됨
- `> 인용` 꺾쇠가 그대로 남음

이 패턴이 보이면 AI가 생성한 마크다운을 그대로 붙여넣은 것이다. 일반 텍스트로 쓸 때는 마크다운 문법을 전부 제거할 것.

### 왜 이게 중요한가

X, 스레드, 링크드인에서 `**`가 그대로 노출된 글은 AI가 쓴 글의 가장 확실한 신호다. 사람이 직접 쓴 글에는 마크다운 문법이 절대 섞이지 않는다.

### 규칙

- 일반 텍스트 환경(SNS, 메신저, 이메일)에서는 마크다운 문법을 전부 벗길 것
- 볼드 강조가 필요하면 플랫폼 고유 서식을 쓸 것 (있는 경우)
- 강조가 꼭 필요한지부터 의심할 것. 대부분 필요 없다
- 불릿 포인트 대신 문장으로 풀어 쓸 것
